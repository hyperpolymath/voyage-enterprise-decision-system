...
VoID (Vocabulary of Interlinked Datasets) is an RDF vocabulary for
describing datasets and their relationships. VEDS uses VoID to:

[arabic]
. *Describe datasets* - Metadata about our transport network, decisions,
tracking data
. *Enable discoverability* - Allow external systems to find and
understand VEDS data
. *Support federation* - Link to external datasets (ports, countries,
carbon factors)
. *Document provenance* - Track where data comes from
...
[arabic]
. *Crawler discovery* - Search engines can find our datasets
. *Federation* - Other systems can understand our schema
. *Tool support* - VoID-aware tools can generate queries
. *Documentation* - Human-readable dataset descriptions
...
VoID statistics are regenerated: - *Hourly* for approximate counts -
*Daily* for full dataset analysis - *On-demand* via admin API
