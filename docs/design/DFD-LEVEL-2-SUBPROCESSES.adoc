...
Level 2 decomposes each Level 1 process into its constituent
sub-processes. This document covers all 8 major processes.
...
[width="100%",cols="13%,16%,18%,21%,32%",options="header",]
|===
|ID |Name |Input |Output |Technology
|1.1 |API Connectors |External API calls |Raw responses |Elixir
HTTPoison, Tesla

|1.2 |Schema Normalizer |Raw JSON/XML/EDI |Normalized JSON |Elixir,
custom parsers

|1.3 |Data Validator |Normalized records |Validated records |Elixir Ecto
changesets

|1.4 |Graph Builder |Validated records |Graph mutations |SurrealDB
client
