...
Julia provides the analytics and visualization backbone for VEDS. Its
strengths:

[arabic]
. *Performance* - Near-C speed for numerical computing
. *Ecosystem* - Makie.jl for publication-quality graphics
. *Interactivity* - Pluto.jl notebooks for exploration
. *Web* - Genie.jl for dashboards
. *Graphs* - Graphs.jl + GraphMakie.jl for network visualization
