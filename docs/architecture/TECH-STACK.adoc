...
Based on discussion and requirements, VEDS uses the following technology
stack:
...
[width="100%",cols="34%,23%,23%,20%",options="header",]
|===
...
[arabic]
. *API Authentication:* JWT tokens, API keys via Phoenix Guardian
. *Database Access:* Network isolation, credentials in secrets manager
. *Inter-Service:* mTLS between containers
. *Audit Trail:* XTDB provides immutable history
. *Formal Verification:* Proofs ensure constraints cannot be bypassed
...
*VEDS differentiation:* Formal verification + labor ethics + open source
+ bitemporality
