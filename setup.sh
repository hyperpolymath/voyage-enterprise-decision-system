...
# setup.sh — Universal setup script for voyage-enterprise-decision-system
#
# Detects your shell, platform, and installs prerequisites.
# Then hands off to `just setup` for project-specific configuration.
#
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/hyperpolymath/voyage-enterprise-decision-system/main/setup.sh | sh
#   # or after cloning:
#   ./setup.sh
#
...
# ── Main ──
main() {
    printf "%s=== voyage-enterprise-decision-system Setup ===%s\n\n" "$BOLD" "$RESET"
    ...
    if [ ! -f "Justfile" ] && [ ! -f "justfile" ]; then
        ...
        info "Clone first: git clone https://github.com/hyperpolymath/voyage-enterprise-decision-system.git"
        info "Then: cd voyage-enterprise-decision-system && ./setup.sh"
        exit 1
    fi
